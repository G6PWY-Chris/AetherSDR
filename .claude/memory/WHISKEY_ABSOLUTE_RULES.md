...
10. **GOLDEN RULE — SSH before internet:** Before doing ANY web browsing, GitHub API calls, or external network research, SSH to one of Chris's local machines first to save Claude Pro tokens. **Use foxtrot (192.168.40.101) — confirmed working.** dev=192.168.40.100 (this machine), ubuntu01=192.168.40.11 (key issue). Run `gh`, `curl`, or `wget` on foxtrot. Only fall back to local WebFetch/WebSearch if no SSH host is reachable.
